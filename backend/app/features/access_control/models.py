...
class GeoException(BaseDocument):
    """A temporary BYPASS of the Geo Fence restriction for one employee — not a second,
    competing location restriction. It carries no latitude/longitude/radius of its own:
    while active, it exempts the employee from the distance check entirely (see
    `app/features/geo_fencing/enforcement.py`), regardless of which active `GeoFence`
