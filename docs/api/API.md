...
- `POST /loan-cases/{id}/new-customer-details` — body `{ preferred_bank_name?, preferred_branch?, loan_type?, requested_amount?, preferred_remarks? }`, all optional; valid only at `new_customer`, advances to `credit_evaluation` (decision 132). Replaces the old bodiless `_PLAIN_TRANSITIONS` move for this pair — closes the one-click, zero-data instant-transition bug.
- `POST /loan-cases/{id}/rv-ov-ref` — body `{ rv_ov_ref_type, rv_ov_ref_status, rv_ov_ref_date, rv_ov_ref_verified_by, rv_ov_ref_result, rv_ov_ref_remarks? }`; valid only at `rv_ov_ref`, advances to `esign_nach_kyc` (decision 130). The old `POST /loan-cases/{id}/bank-details` (decision 061, single-slot Bank/NBFC fields) was removed in decision 130 — dead since decision 129 superseded it with multi-bank-offer support and no frontend ever called it.
