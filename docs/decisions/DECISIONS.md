...
## 128 — Four production bug fixes in Module 6A (Leads), found during real manual post-deployment testing: Owner self-assignment, duplicate-mobile enforcement, follow-up sort order, and a Reject-visibility false alarm

**Decision:** All four fixes are bug fixes under the freeze policy (decision 031) — no new capability, no schema repurposing, no second parallel mechanism introduced:

1. **Owner "Assign To: Self" fixed.** `LeadService._resolve_assignee`/`_assign` previously assumed every non-empty `assigned_to` resolves through `EmployeeRepository`, which is correct for an Employee but an Owner has no Employee record by design — so an Owner using "Self" on Create Lead or Edit Lead's inline `assigned_to` always raised "You have no employee record to assign a lead to." Fixed by letting `assigned_to` hold the Owner's own `User` id in that one case (mirroring the existing `Lead.created_by` id-space, which already holds a `User` id while `assigned_to` normally holds an `Employee` id) — `_assign` now checks `UserRepository.find_by_id(assignee_id).role == OWNER` before falling back to the `Employee` lookup, deliberately **not** via `OwnerProfileRepository` (an `OwnerProfile` row is optional profile detail a legacy/grandfathered Owner account may lack — confirmed live: this repo's own local dev Owner has no `OwnerProfile` row at all — while `User.role` is the one field guaranteed present for every Owner). `resolve_names` gained a matching branch so an Owner-held `assigned_to` resolves to a real display name (falling back to the generic "Owner" label already used for `assigned_by`/`rejected_by` when no `OwnerProfile` row exists), never a raw id. No new Employee record is ever created for an Owner; no second assignment mechanism was introduced — `assigned_to` remains the one field.
2. **Duplicate-mobile creation now blocked server-side.** `create_lead` previously only ever flagged a repeat mobile (`is_potential_duplicate`/`duplicate_of_lead_ids`) and let creation proceed regardless. It now raises `ConflictError` (409, `details.existing_lead_id`/`existing_lead_code`) when an **active** (non-rejected) lead already holds that mobile — a mobile may still legitimately recur across a rejected lead and a new active one (the existing flag-only behavior is preserved for exactly that case). A **database-level** safety net was added to close the create-time-check-then-insert race: `scripts/migrate_add_lead_mobile_unique_index.py`, a new, explicit, non-destructive migration — it first inspects for any pre-existing mobile shared by more than one active lead and, if found, only reports them and creates nothing (never deletes/merges data); only once the data is clean does it create a **partial** unique index (`{"rejected_at": None}`, since Mongo partial indexes support `$eq`/`$exists`/`$gt(e)`/`$lt(e)`/`$type`/`$and` only — not `$ne`, so `rejected_at: None` is used instead of `stage: {"$ne": "rejected"}`, semantically identical). Run locally against `afs_crm`: zero pre-existing conflicts found, index created successfully. **Must be run against the production database before this deploys** (not run automatically by application startup, and not run against production by this session, per the explicit "do not modify production" instruction).
3. **Fresh Leads (and every tab) follow-up ordering fixed.** No tab's list query ever applied an explicit sort when the caller sent none (the frontend never has) — results fell back to MongoDB's natural/insertion order, not the Past/Today/Future/No-Date priority the UI's own color coding (red/blue/green) implies. `LeadRepository.search_and_filter` now branches: an explicit `sort` (e.g. CSV export's `created_at desc`) is unchanged; otherwise a new aggregation (`_find_with_follow_up_priority`) computes a `_follow_up_rank` (`0`=past, `1`=today, `2`=future, `3`=no date, using the same `start_of_day_ist`/`end_of_day_ist` boundaries the color coding itself uses) and sorts by `(rank, next_follow_up_date, created_at desc)` entirely **before** `$skip`/`$limit`, so pagination can never see a partially-sorted page. This changes the default ordering for every tab with no explicit sort (not just Fresh Leads) — judged correct and desirable since My Leads/Assigned also render Next Follow Up, not a scope overreach.
4. **Reject button "missing" from My Leads' Update form: investigated, no code change — a false alarm, most likely stale deployed permissions.** `UpdateStageModal.tsx`'s Reject button and the backend `POST /leads/{id}/reject` gate are both exactly as decision #126 left them (that decision's own fix for the `_MY_PERMISSIONS_CATALOG` gap is confirmed still present, `access_control/service.py`'s `leads:leads` tuple still includes `"reject"`) — re-verified end-to-end (backend tests + a real local-browser Owner session) that the button renders, opens the existing `RejectLeadModal`, and completes the reject flow (stage → `rejected`, `rejected_reason`/`rejected_by`/`rejected_at` set, lead leaves My Leads, tab counts update, lead appears in Rejected). Likely explanation for the field report: the account tested in production had not actually been granted the `reject` action on its role (a genuine, separate permissions-configuration gap, not a code defect), or `scripts/migrate_backfill_lead_stage.py` (which `$addToSet`s `reject` onto the seeded `leads:leads` `Permission` document) was never run against that environment. Recommendation: confirm the affected account's role grants `leads:leads:reject`, and confirm that migration has run in production.

**Why:** User report of four specific post-deployment issues from real manual testing, with an explicit "fix only these four, do not start a new phase, do not redesign Leads" scope.

**Impact:** `backend/app/features/leads/{repository,service}.py` (additive: `UserRepository`/`OwnerProfileRepository` now consumed read-only, one new private sort helper, `ConflictError` on create); new `scripts/migrate_add_lead_mobile_unique_index.py`. Frontend: `frontend/src/features/leads/pages/CreateLeadPage.tsx` (409-specific duplicate-blocked banner with a link to the existing lead — Edit Lead's equivalent banner was out of this fix's scope since the report was specifically about Create). Tests: `tests/api/test_leads.py` (+17: Owner self-assign via Create and Edit, My Leads visibility, Employee self-assign and Employee-to-Employee regression checks, Owner-assigned-lead name resolution, duplicate-mobile blocked with `details` payload, rejected-duplicate still allowed and still flags, follow-up priority ordering including a pagination-boundary check, Reject permission-gated end-to-end flow with tab-count assertions) — one pre-existing test (`test_duplicate_detection_flags_but_never_blocks`) rewritten to `test_duplicate_detection_blocks_second_active_lead_for_same_mobile` since it asserted the exact behavior this entry replaces. Full backend suite and frontend `tsc -b --noEmit`/`eslint`/`vite build` all run clean; manual verification performed against a real local dev session (Owner login, live browser) for all four fixes.

**Freeze (reconfirmed):** Modules 6A/6B/6C remain frozen for anything beyond what decisions #125–#128 list. This is a bug-fix entry under the existing freeze policy, not a new phase of the Leads redesign.

---

# Loan Management Workflow Redesign

## 129 — Loan Management's pipeline redesigned to an exact 12-status list (adding RV/OV/Ref and Re-Eligible, retiring `documents_pending` from the mandatory path), plus genuine multi-bank-offer support replacing the old single-slot Bank/NBFC fields

**Decision:** Module 6C's Loan pipeline (frozen per decisions #060–#064) gains, in `workflow_engine`/`loan_management` only — Insurance Management is untouched, confirmed by its own test suite passing unmodified:

- `LoanStatus` gains `RV_OV_REF`/`RE_ELIGIBLE`. `DOCUMENTS_PENDING` stays defined (old `ApplicationStatusHistory` rows reference it as a plain string, and the model layer never constrained `current_status`) but is removed from `RESUMABLE`/`ALL` — no new case can ever be placed into it again. It used to sit mandatorily between `NEW_CUSTOMER` and `CREDIT_EVALUATION`; that position is gone because a Lead only ever reaches Loan Management once its required documents are already verified (decision #127), making a second, Loan-Management-internal "wait for documents" gate before Credit Evaluation redundant. `request_documents`/`verify_documents` remain as optional, non-pipeline-driving actions at `new_customer`/`additional_documents` (retargeted to land on `credit_evaluation`/`rv_ov_ref` respectively when used) — the mandatory pipeline itself now always advances via `LoanCaseService.update_status`'s plain-transition dispatch instead.
- New pipeline: `new_customer → credit_evaluation → (bank-offer select) → offer_acceptance → (explicit confirm) → additional_documents → rv_ov_ref → esign_nach_kyc → final_evaluation → send_for_disbursement → disbursed`, with `credit_evaluation ⇄ re_eligible` and `credit_evaluation/offer_acceptance/final_evaluation/re_eligible → rejected` as the side branches, and the existing, fully-reused, unmodified On Hold/Resume mechanism (`workflow_engine/hold.py`, `HoldReason`) available from every non-terminal status. `REJECTED` stays in `LoanStatus.TERMINAL` (unchanged) — `leads/service.py`'s `list_eligible_assignees` relies on that meaning "permanently closed" for workload balancing, so `RE_ELIGIBLE` is reachable only from `CREDIT_EVALUATION`, never automatically from a bank-level decision and never directly from `REJECTED`.
- **Select vs. Accept are two distinct, deliberately separate steps** (explicit business requirement): `select_bank_offer`/`select_bank_offer_as_customer` only choose which bank offer the case proceeds with — marking it `is_selected`, copying its `approved_amount` onto `loan_details.offered_amount` (keeping the `_loan_revenue`/`_monthly_revenue`/`_revenue_trend_chart` dashboard widgets, which read that exact field, working unchanged), and moving the case to `offer_acceptance`. A separate, explicit `confirm_offer_acceptance`/`confirm_offer_acceptance_as_customer` action is the only thing that actually advances the case to `additional_documents` — selecting an offer never silently skips this audit checkpoint.
- New `LoanCaseBankOffer` entity (`backend/app/features/loan_management/models.py`, this module's first models file — bank offers are Loan-only, so they belong in the product module rather than the shared `workflow_engine`, unlike `ApplicationWorkflow`/`LoanCaseDetails` which stay shared with Insurance): `loan_case_id`, `bank_name`, `bank_application_id`, `reference_number`, `assigned_officer`, `decision` (`approved`/`rejected_re_eligible`), `approved_amount` (required at the schema layer iff approved, always null otherwise), `remarks`, `is_selected`/`selected_at`/`selected_by`. Any number of offers per case; adding one never overwrites another; exactly one can ever be `is_selected` (enforced by unselecting any prior selection before setting a new one, service-layer, never at the schema level). **Supersedes decision #061** — the old single-slot `LoanCaseDetails.bank_nbfc_name`/`bank_decision`/etc. fields are left in place, untouched, for historical reads of old cases, but no longer written by the Credit Evaluation flow going forward.
- **Removes** `record_offer`/`accept_offer`/`decline_offer` (service, router, schemas) entirely — **superseding decision #062**, which had already documented these as built but with zero frontend ever wired to them; they operated on `LoanCaseDetails`'s single `offered_amount` slot, structurally incompatible with multiple simultaneous bank offers, and keeping both would itself have been the "second parallel offer mechanism" this redesign exists to avoid. Replaced by the new bank-offer select/confirm endpoints, this time with an actual Customer Portal screen (a genuine gap decision #062 had left open).
- `credit_evaluation()` (the case-level credit score/remarks action) no longer transitions the case itself — it's now pure data capture, independent of any individual bank's own decision, matching the same "case status and bank decision are separate concepts" principle bank offers themselves are built on.
- New `GET /loan-cases/counts` (one count per `LoanStatus.ALL` value, same `assigned_to`-scoped, server-computed, list-call-consistent pattern Leads' `GET /leads/counts` established, decision #125) backs 12 live tab badges in a redesigned `LoanManagementLayout.tsx` (previously 4 tabs, one of which — "Re-Eligible" — was an already-shipped empty-shell placeholder with no backing status; it now shows real data for the first time). Status labels consolidated into one canonical `frontend/src/features/loan_management/constants.ts`, replacing a verbatim-duplicated map that previously lived in both `LoanCaseListPage.tsx` and `LoanCaseDetailsPage.tsx`.
- The existing time-based cooldown reminder (`worker/tasks/reminders.py::check_re_eligible_cases`, which notifies on `REJECTED` cases after `eligible_after_days` without ever touching `current_status`) is unrelated and untouched — a deliberate choice, confirmed with the user: the new persisted `re_eligible` *status* and the existing re-eligibility *reminder* are two different concepts that happen to share a name, kept as independent mechanisms rather than merged.

**Why:** User-specified redesign of the Loan Management pipeline to an exact 12-status list with genuine multi-bank-offer support — the existing architecture had no concept of more than one bank/NBFC offer per case at all. Delivered by reusing the existing `WorkflowEngine`/`ApplicationStatusHistory`/Hold/audit mechanisms verbatim (zero new transition, audit, or status-history machinery), and by minimizing the actual status-list delta: 9 of the 12 target statuses already existed under the same name, so only two genuinely new statuses were added and one (`documents_pending`) was retired from the mandatory path rather than the pipeline being rebuilt.

**Impact:** Backend — `backend/app/features/workflow_engine/{constants,repository}.py` (additive: two new statuses/audit events, one new `BankOfferDecision` enum, one new `count_filtered` repository method); `backend/app/features/loan_management/{service,router,schemas,mappers}.py`; new `backend/app/features/loan_management/{models,repository,indexes}.py` (this module's first models/repository/indexes files); `backend/app/main.py` (wires the new index); `scripts/seed.py` (updated Loan `WorkflowDefinition` row tuples — a fresh-database-only change); new `scripts/migrate_redesign_loan_pipeline.py` (the production migration — `$set`-updates the four existing rows whose transition graph changed, upserts the two new rows, and reports, without ever auto-applying, any case still sitting in the retired `documents_pending` status). Frontend — new `frontend/src/features/loan_management/constants.ts`; `frontend/src/features/loan_management/{statusControl,api}.ts`; `frontend/src/features/loan_management/pages/{LoanManagementLayout,LoanCaseListPage,LoanCaseDetailsPage}.tsx`; new `frontend/src/features/loan_management/components/{CreditEvaluationBankOffers,OfferAcceptancePanel}.tsx`; new customer-facing bank-offer selection page under `frontend/src/features/customer/pages/`. Tests: `tests/api/test_case_status_control.py`, `tests/api/test_workflow.py` (both updated for the new transition graph), new `tests/api/test_loan_bank_offers.py`. Insurance Management: untouched — its own test suite passes unmodified, confirming the shared `workflow_engine` engine/hold/history code carries no Loan-specific logic.

**Freeze (reconfirmed):** Module 6C remains frozen for anything beyond what decisions #060–#064/#129 list — any further change to the Loan or Insurance pipeline needs its own new decision entry and explicit approval, the same as any other frozen-module change from here on.

## 130 — Loan Management eligibility gate: a case must not be visible/actionable until explicitly moved out of Document Collection; RV/OV/Ref gains real stage data; the dead single-slot `bank-details` endpoint is removed; View/Update are split on the frontend

**Decision:** A production audit found that a Loan Case (`ApplicationWorkflow`, `case_type="loan"`) was created and made fully visible/actionable in Loan Management's list, counts, and detail endpoints the instant `CustomerService.submit_application` set `Application.status=submitted` — checking only that required documents were *uploaded*, never staff-*verified* — entirely independent of `LeadService.set_stage`'s `loan_management` branch (decision #127), which already implements the correct eligibility check (submitted + all required documents verified) but, until now, only relabelled the Lead's own `stage` field and never touched the case at all.

- New field `ApplicationWorkflow.moved_to_loan_management_at: datetime | None`. `None` means "not yet eligible to appear in Loan Management." Loan-only; Insurance never sets or reads it.
- For a **lead-less application** (Flow 2 — a customer registers and applies directly; no Lead/Document Collection pipeline exists to gate it through, and therefore no "Move to Loan Management" action will ever fire for it), `LoanCaseService._create_case_for_application` sets this field immediately at creation — unchanged, always-visible behavior, exactly as before this fix.
- For a **Lead-originated application** (Flow 1), the field stays `None` until `LeadService.set_stage`'s `loan_management` branch — after its existing checks pass — calls the new `LoanCaseService.mark_moved_to_loan_management` (idempotent; get-or-creates the case via the existing `ensure_case_for_application`, sets the timestamp, writes a new `LoanAuditEvent.MOVED_TO_LOAN_MANAGEMENT` audit entry). Only fires when `application.product_category == "loan"` — Insurance's own visibility is untouched, unaffected by this decision.
- `LoanCaseService.list_cases`/`get_counts` filter on `moved_to_loan_management_at != None` via a new generic, opt-in `extra_filter` kwarg added to `ApplicationWorkflowRepository.search_and_filter`/`count_filtered` (unused by Insurance's own calls — zero behavior change there). `get_case`/`get_own_case` additionally raise `NotFoundError` for an un-moved case — for every actor except the Owner on the staff path (deliberately exempted, for admin/troubleshooting visibility).
- **Production migration required**: existing cases predate this field and must be grandfathered, not hidden — staff have been legitimately working them regardless of the Lead-stage/case decoupling this decision closes, and retroactively gating on `lead.stage == "loan_management"` would make currently-in-progress cases vanish, a regression rather than a fix. `scripts/migrate_backfill_moved_to_loan_management.py` sets `moved_to_loan_management_at = created_at` on every existing case_type="loan" row missing the field (idempotent, non-destructive). Must run once before the new backend deploys.
- **RV/OV/Ref gains real stage data** (it previously captured nothing — a bare, bodiless entry in `LoanCaseService._PLAIN_TRANSITIONS`): six new `LoanCaseDetails` fields (`rv_ov_ref_type/status/date/verified_by/result/remarks`), new `RvOvRefRequest` schema, new `LoanCaseService.record_rv_ov_ref` (valid only at `current_status == rv_ov_ref`; transitions to `esign_nach_kyc`), new `POST /loan-cases/{case_id}/rv-ov-ref`. `(RV_OV_REF, ESIGN_NACH_KYC)` removed from `_PLAIN_TRANSITIONS` — like every other forward move carrying mandatory business data, it now requires the dedicated action.
- **`allowed_next_statuses`** added to `LoanCaseListItem`/`LoanCaseDetailResponse`, sourced from `WorkflowDefinition.allowed_next_statuses` for the case's current status (`LoanCaseService.status_transition_map`, batch-fetched once per request) — lets the frontend reflect the backend's real, configured transition graph rather than relying solely on the hand-maintained `statusControl.ts` hint.
- **Removed dead code**: `POST /loan-cases/{case_id}/bank-details`, `BankDetailsRequest`, `LoanCaseService.record_bank_details`, frontend `recordBankDetails` — confirmed zero test coverage and zero frontend callers; already conceptually superseded by decision #129's multi-bank-offer-at-Credit-Evaluation design, this removes the actual dead code left behind by that supersession. New Customer's real stage-specific action stays its existing Document Verification request/verify flow — not a resurrected single-bank-slot form, which would have contradicted #129.
- **Frontend**: `CaseListPage.tsx` (shared with Insurance) gains an optional `onUpdate` prop, used only by `LoanCaseListPage.tsx`, adding a distinct Update action beside View without changing Insurance's rendered output at all. A new `UpdateLoanCaseModal.tsx` extracts the stage-progression UI that used to render unconditionally inline on `LoanCaseDetailsPage.tsx`; that page now renders stage data read-only, with Assign/Hold-Resume/Notes kept as separate, always-visible meta-actions (they're orthogonal to the linear stage pipeline). No routing changes — `/loan-management` and `/loan-cases` were already two URL entry points to the same components, not competing implementations; confirmed via full audit, nothing to remove.

**Why:** The core complaint mapped to one confirmed, real bug — case visibility decoupled from the Document Collection handoff it was supposed to gate on — not a "generic status form" architecture problem; the pipeline, per-stage dedicated endpoints, audit trail, and RBAC were already correctly built (decision #129). Fixing it as an additive gating field (rather than moving case-creation timing itself) keeps the frozen Module 6B submission path and every existing lazy-creation test untouched, at the cost of one new field and one new migration.

**Impact:** Backend — `backend/app/features/workflow_engine/{models,constants,engine,repository}.py`; `backend/app/features/loan_management/{service,router,schemas,mappers}.py`; `backend/app/features/leads/service.py` (`set_stage`); new `scripts/migrate_backfill_moved_to_loan_management.py`. Frontend — `frontend/src/components/pages/CaseListPage.tsx`; `frontend/src/features/loan_management/{api,statusControl,constants}.ts`; `frontend/src/features/loan_management/pages/{LoanCaseListPage,LoanCaseDetailsPage}.tsx`; new `frontend/src/features/loan_management/components/UpdateLoanCaseModal.tsx`. Tests: `tests/api/test_leads.py`, `tests/api/test_case_status_control.py`, new eligibility-gate and RV/OV/Ref coverage. Insurance Management: untouched by design — confirmed via its own test suite and via the `extra_filter`/`onUpdate` additions both being no-ops when unused.

## 131 — Loan Management's own View action stops linking to the legacy `/loan-cases/:id` back-compat route; new canonical `/loan-management/cases/:caseId`

**Decision:** Added `{ path: "/loan-management/cases/:caseId", element: <LoanCaseDetailsPage /> }` (`frontend/src/app/router.tsx`) — the exact same `LoanCaseDetailsPage` component `/loan-cases/:caseId` already rendered, not a second implementation. `LoanCaseListPage.tsx`'s `detailBasePath` changed from `/loan-cases` to `/loan-management/cases`, so every View link Loan Management's own list renders now targets the canonical path. `LoanCaseDetailsPage.tsx`'s own Back button (`useDocumentCollectionBackContext`) defaults to `/loan-management/cases` for the same reason. `StaffApplicationDetailsPage.tsx`'s "Manage Status →" link and `CustomerDetailsPage.tsx`'s case link were also repointed, for consistency (same component either way, one-line change each). `/loan-cases` and `/loan-cases/:caseId` remain registered — still legitimate back-compat for bookmarks/dashboard widgets — but nothing in Loan Management's own UI links there anymore.

**Why:** A production audit found Loan Management's "View" button opened `/loan-cases/:caseId`, not a Loan-Management-namespaced route — confusing given the sidebar's own primary entry point is `/loan-management`. Not a competing-implementation bug (same component, same data, same permissions), but a URL-namespace inconsistency worth closing.

**Impact:** `frontend/src/app/router.tsx`, `frontend/src/features/loan_management/pages/{LoanCaseListPage,LoanCaseDetailsPage}.tsx`, `frontend/src/features/customer/pages/{StaffApplicationDetailsPage,CustomerDetailsPage}.tsx`, `frontend/src/shared/navigationContext.test.tsx`. No backend change. Insurance Management untouched (`/insurance-cases/:caseId` unaffected).

## 132 — New Customer becomes a real dedicated action (not a bodiless one-click transition); every plain status-control action gets an explicit confirm step; Additional Documents' checklist is scoped to the case's own product; bank offers gain Interest Rate/Tenure/Processing Fee

**Decision:** A production audit found `new_customer -> credit_evaluation` was still a bodiless `_PLAIN_TRANSITIONS` move — the Update modal's single "Update Status" button fired the transition immediately on click, with zero fields and zero confirmation step (the literal "opens Update, fills nothing, status changes" bug). Fixed by:

- **New Customer dedicated action**: 5 new optional `LoanCaseDetails` fields — `preferred_bank_name`, `preferred_branch`, `loan_type`, `requested_amount`, `preferred_remarks` (deliberately NOT reusing `bank_nbfc_name`/etc., which decision #129 already dedicated exclusively to the later bank-offer-selection flow — writing both concepts to the same field at two different stages would be ambiguous). New `NewCustomerDetailsRequest` schema (all fields optional — this is an initial intake preference, not a hard gate), new `LoanCaseService.record_new_customer_details` (valid only at `current_status == new_customer`, saves the fields, transitions to `credit_evaluation`), new `POST /loan-cases/{case_id}/new-customer-details`. `(NEW_CUSTOMER, CREDIT_EVALUATION)` removed from `_PLAIN_TRANSITIONS` — like every other transition carrying real business data, it now requires the dedicated action. `frontend/src/features/loan_management/statusControl.ts`'s `new_customer` entry changed from `"simple"` to `"dedicated"`; a new `NewCustomerDetailsForm` renders directly (mirrors `RvOvRefForm`'s existing pattern) whenever `status === "new_customer"`.
- **Every simple status-control action now requires an explicit confirm step, not only Reject**: `UpdateLoanCaseModal.tsx`'s `StatusUpdateControl` generalized its `rejectingIndex`/`rejectReason` state into `confirmingIndex`/`confirmRemarks` — clicking any simple action's button (Mark Re-Eligible, Move to Credit Evaluation from Re-Eligible, Reject) only reveals an inline panel (Remarks optional, mandatory only for Reject) with explicit "Confirm"/"Cancel" buttons; nothing calls the backend until "Confirm" is clicked, and "Cancel" (or closing the modal) leaves the case completely untouched. Combined with New Customer no longer being bodiless at all, this closes every remaining one-click-instant-mutation path.
- **Additional Documents' checklist is scoped to the case's own product**: `UpdateLoanCaseModal.tsx`'s document-request section previously sourced `documentTypesApi.list()` — the entire system-wide document-type catalog, the same one Document Collection/Document Verification uses, wrongly reused here. Replaced with `getFormDefinition("loan", loanCase.product_id).required_documents` (`@/features/customer/api`, already open to any authenticated user), filtered to non-hidden, required entries, each already carrying its own display name — no separate document-type lookup needed. The `documentTypes` prop threaded down from `LoanCaseListPage.tsx`/`LoanCaseDetailsPage.tsx` was removed entirely; the modal fetches its own scoped list internally.
- **Bank offers gain Interest Rate, Tenure (months), and Processing Fee** (`LoanCaseBankOffer`, `BankOfferRequest`/`BankOfferResponse`, `add_bank_offer`/`update_bank_offer` — both explicitly enumerate fields rather than spreading the payload, so the 3 new fields needed adding to each method body, not only the schema). Selecting a bank offer additionally copies `interest_rate`/`tenure_months` into `LoanCaseDetails`'s pre-existing, previously-always-null `offered_interest_rate`/`offered_tenure_months` fields.
- **Tab-count/list consistency audited, confirmed not a bug**: `list_cases`/`get_counts` share identical `_LOAN_MANAGEMENT_GATE`/`assigned_to` filtering — the only way the list can show fewer rows than a status badge is the Owner-only "Unassigned Cases only" checkbox, an intentional additional narrowing filter (the same category as the search box), not a data-consistency defect. New regression tests lock in base-case (no extra filters) list/count agreement instead of changing code.

**Why:** The core complaint was a genuine, reproducible bug (New Customer's bodiless transition) plus two real data-source/data-model gaps (the wrong document catalog; missing bank-offer fields) — not evidence of a deeper architectural problem. Generalizing the already-correct Reject confirm-panel pattern closes every remaining unsafe path with a small, surgical change, rather than rearchitecting the already-tested Bank Offer add/select/confirm flow.

**Impact:** Backend — `backend/app/features/workflow_engine/{models,constants}.py`; `backend/app/features/loan_management/{service,router,schemas,mappers,models}.py`. Frontend — `frontend/src/features/loan_management/{api,statusControl}.ts`; `frontend/src/features/loan_management/components/{UpdateLoanCaseModal,CreditEvaluationBankOffers}.tsx`; `frontend/src/features/loan_management/pages/{LoanCaseListPage,LoanCaseDetailsPage}.tsx` (removed `documentTypes` prop threading). Tests: ~12 existing setup call-sites across `tests/api/{test_case_status_control,test_workflow,test_loan_bank_offers}.py` switched from the now-removed plain PATCH to the new dedicated endpoint; new New-Customer/regression/bank-offer/count-consistency tests; new frontend mutation-guard tests. No migration required — every new field is optional and additive. Insurance Management untouched.
