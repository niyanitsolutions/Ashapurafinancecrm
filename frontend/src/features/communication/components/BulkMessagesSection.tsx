  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <button type="button" onClick={() => setShowComposer((v) => !v)} className="rounded bg-primary text-white text-sm font-medium py-2 px-4">
          {showComposer ? "Cancel" : "+ New Bulk Message"}
        </button>
      </div>
      ...
      <div className="bg-card border border-border rounded-card shadow-card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {jobs.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-6 text-center text-text/50">No bulk messages yet.</td>
              </tr>
            )}
            {jobs.map((job) => (
...
function BulkMessageComposer({
  ...
  return (
    <div className="mb-6 bg-card border border-border rounded-card shadow-card p-6 space-y-4">
      ...
      <div>
        ...
        <div className="flex gap-3 mb-2">
          ...
          />
          <button type="button" onClick={onSearch} disabled={isSearching} className="rounded-lg border border-primary text-primary text-sm font-medium py-2 px-4 disabled:opacity-50">
      ...
      <div className="flex justify-end">
        <button
          type="button" disabled={!templateId || selected.size === 0} onClick={onSubmit}
          className="hover-lift rounded-lg bg-gradient-to-r from-primary to-primary-dark text-white text-sm font-semibold py-2 px-5 shadow-card disabled:opacity-50"
        >
          Queue Bulk Message
        </button>
      </div>
...
function BulkJobDetailPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-lg bg-card border border-border rounded-2xl shadow-dropdown p-6">
        <h2 className="text-lg font-bold text-text mb-3">Bulk Send Result</h2>
        ...
        {job && (
          <>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <button type="button" onClick={onViewFailed} className="rounded border border-border text-sm font-medium py-2 px-3.5 text-text hover:bg-background">
                View Failed
              </button>
              {job.failed > 0 && (
                <button
                  ...
                  className="rounded border border-primary text-primary text-sm font-medium py-2 px-3.5"
                >
                  Retry Failed
                </button>
              )}
              {(job.status === "queued" || job.status === "processing") && (
                <button
                  type="button"
                  onClick={() => run(() => cancelBulkMessageJob(jobId), "Bulk job cancelled.", () => { load(); onChanged(); })}
                  className="rounded border border-danger/30 text-danger text-sm font-medium py-2 px-3.5"
                >
                  Cancel
                </button>
              )}
        ...
        <div className="flex justify-end">
          <button type="button" onClick={onClose} className="rounded-lg bg-gradient-to-r from-primary to-primary-dark text-white text-sm font-semibold py-2 px-5 shadow-card">
            Close
          </button>
        </div>
      </div>
